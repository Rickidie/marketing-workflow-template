...
### Optional X/Twitter discourse check

Use this only when the brief, audience, or Content Calendar explicitly needs X/Twitter discourse.

If [TweetClaw](https://github.com/Xquik-dev/tweetclaw) or another approved collector is available, treat it as source intake only. Capture a source packet with:

- Query or handle used
- Public post URLs and capture time
- Short raw excerpts or reply themes
- Visible engagement context when relevant
- Collection method and any filtering boundary

Do not let source intake draft, schedule, publish, reply, message, or change account state. The trend snapshot should cite useful public discourse, then the content creator and reviewer remain responsible for the final content decision.
